...
pub(crate) fn ec_curve_from_private_key_der(private_key: &[u8]) -> Option<EcdsaCurve> {
    ...
    let p256k_oid = pkcs8::ObjectIdentifier::from_der(&SECP256K1_OID.to_der_vec().ok()?).ok()?;
    ...
    if ec_key.algorithm.assert_parameters_oid(p256_oid).is_ok() {
        return Some(EcdsaCurve::P256);
    } else if ec_key.algorithm.assert_parameters_oid(p256k_oid).is_ok() {
        return Some(EcdsaCurve::P256K);
